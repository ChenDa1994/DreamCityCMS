<template>
    <div class="dr-adminGroupForm">
        <ItemForm :formState="booksItemForm" />
        <el-form :model="formState.formData" :rules="rules" ref="ruleForm" label-width="120px" class="demo-ruleForm">
            ...
            <div v-if="formState.formData.type == '1'">
                ...
                <el-form-item label="图片列表" prop="items">
                    <el-button size="small" type="primary" plain round @click="showBooksItemForm">添加图片</el-button>
                    <div class="dr-books-item" v-for="item in formState.formData.items" :key="item._id">
                        <div class="img">
                            <img :src="item.sImg" />
                        </div>
                        <div class="details">
                        ...
                        <div class="options">
                            <el-button size="mini" type="primary" plain round @click="editBooksItemInfo(item)">
                                <i class="fa fa-edit"></i>
                            </el-button>
                        </div>
                        <i class="el-icon-close" @click="deleteBooksItem(item)"></i>
                    </div>
                </el-form-item>
            </div>
            <div v-if="formState.formData.type == '0'">
                <el-form-item label="链接列表" prop="items">
                    <el-button size="small" type="primary" plain round @click="showBooksItemForm">添加链接</el-button>
                    <div v-if="formState.formData.items.length > 0">
                        <el-tag v-for="tag in formState.formData.items" :key="tag.title" type='gray' :closable="true" @close="deleteBooksItem(tag)">
                            <span @click="editBooksItemInfo(tag)">{{tag.title}}</span>
                        </el-tag>
                    </div>
                </el-form-item>
            </div>
...
export default {
    ...
    methods: {
        ...
        showBooksItemForm() {
            this.$store.dispatch('showBooksItemForm', { edit: false })
        },
        editBooksItemInfo(item) {
            this.$store.dispatch('showBooksItemForm', {
                edit: true,
                formData: item
            });
        },
        deleteBooksItem(item) {
            let oldFormState = this.$store.getters.booksInfoForm;
            let booksItems = oldFormState.formData.items;
            let currentIndex = _.findIndex(booksItems, item);
            booksItems.splice(currentIndex, 1);
            this.$store.dispatch('booksInfoForm', oldFormState);
        },
        submitForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    if (this.formState.edit) {
                        services.updateBooks(params).then((result) => {
                            if (result.data.state === 'success') {
                                this.$store.dispatch('hideBooksItemForm');
                                this.$message({
                                    message: '更新成功',
                                    type: 'success'
                                });
                        ...
                        services.addOneBook(params).then((result) => {
    ...
    computed: {
        ...
        formState() {
            return this.$store.getters.booksInfoForm
        }
    },
    mounted() {
        // 针对手动页面刷新
        if (this.$route.params.id) {
            services.getOneBook(this.$route.params).then((result) => {
                if (result.data.state === 'success') {
                    if (result.data.doc) {
