<template>
    <div class="dr-adminGroupForm">
        <el-dialog width="35%" size="small" :title="(formState.edit?'编辑':'添加')+(booksType == '1'?'图片':'文本链接')" :visible.sync="formState.show" :close-on-click-modal="false">
            <el-form v-if="booksType == '1'" :model="formState.formData" :rules="rules" ref="ruleForm" label-width="80px" class="demo-ruleForm">
            ...
            <el-form v-if="booksType == '0'" :model="formState.formData" :rules="rules1" ref="ruleForm1" label-width="80px" class="demo-ruleForm">
...
export default {
    ...
    computed: {
        booksType() {
            return this.$store.getters.booksInfoForm.formData.type
        }
    },
    methods: {
                ...
                if (valid) {
                    ...
                    if (this.formState.edit) {
                        for (let i = 0; i < booksItems.length; i++) {
                            if (booksItems[i]._id == params._id) booksItems[i] = params;
                        }
                        this.$store.dispatch('booksInfoForm', oldFormState);
                    } else {
                        // 新增
                        booksItems.push(params);
                        this.$store.dispatch('booksInfoForm', oldFormState);
                    }
